  ...
  async initializePayment(reservationId: string, userId: string): Promise<PaymentInitializationResponse> {
    try {
      const reservation = await SupabaseClientService.selectFromTable(
        'reservations',
        { id: reservationId },
        'id, amount, guest_email, status, guest_name'
      );
      if (!reservation) {
      ...
      const transaction = await PaymentGatewayService.initializeTransaction(reservation.amount, reservation.guest_email, { reservationId });
      ...
      await SupabaseClientService.updateTable(
        'reservations',
        { payment_reference: transaction.reference, status: 'awaiting_payment' },
        { id: reservationId }
      );
  ...
  async verifyPayment(reference: string): Promise<PaymentVerificationResponse> {
    try {
      ...
      const reservation = await SupabaseClientService.selectFromTable(
        ...
        'id, guest_name, guest_email, room_details'
      );

      if (transaction.status !== 'success') {
        const updatedPayment = await this.paymentRepository.updatePayment(reference, { status: 'Failed' });

        const updatedReservation = await SupabaseClientService.updateTable(
          'reservations',
        ...
        await InvoiceService.sendInvoiceEmail(reservation.id);
